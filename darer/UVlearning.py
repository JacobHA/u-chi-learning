...
class uLearner:
    ...
    def _initialize_networks(self):
        ...
        self.online_vs = OnlineNets([UNet(self.env, hidden_dim=self.hidden_dim, device=self.device)
                                                    for _ in range(self.num_nets)],
                                                     aggregator=self.aggregator)
        self.target_vs = TargetNets([UNet(self.env, hidden_dim=self.hidden_dim, device=self.device)
                                                        for _ in range(self.num_nets)])         
        self.target_vs.load_state_dicts([v.state_dict() for v in self.online_vs])
    ...
    def train(self):
        ...
        for grad_step in range(self.gradient_steps):
            ...
            next_actions = self.online_us.choose_action(next_states)
            next_actions = torch.tensor(next_actions).to(self.device).unsqueeze(-1)
            next_v = torch.cat([online_v(next_states).squeeze().gather(1,next_actions.long())
                                    for online_v in self.online_vs], dim=1)
            next_v = self.aggregator_fn(next_v, dim=1)[0]
            with torch.no_grad():
                online_u_next = torch.stack([u(next_states)
                                    for u in self.online_us], dim=0)
                online_curr_u = torch.stack([u(states).gather(1, actions)
                                    for u in self.online_us], dim=0)
                target_curr_v = torch.stack([v(states).gather(1, actions)
                                    for v in self.target_vs], dim=0)
                curr_v = self.aggregator_fn(target_curr_v, dim=0)[0]
                ...
                expected_next_v = torch.exp(self.beta * (in_exp)) * (1 - dones) * curr_v
                expected_next_v = expected_next_v.squeeze(1)
            ...
            self.logger.record("train/v_loss", v_loss.item())
