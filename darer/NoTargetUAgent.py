...
class NoTargetUAgent(BaseAgent):
    def __init__(self,
                 *args,
                 use_rawlik=False,
    ...
    def exploration_policy(self, state: np.ndarray) -> (int, float):
        with torch.no_grad():
            ...
            if self.use_rawlik:
                pi0 = self.online_prior.nets[0](state).squeeze()
                # pi0 /= pi0.sum()
            else:
    ...
    def gradient_descent(self, batch, grad_step: int):
        ...
        with torch.no_grad():
            ...
            online_curr_ua = torch.stack([u(states)
                                          for u in self.online_us], dim=0)
            if self.use_rawlik:
                online_priora = self.online_prior.nets[0](states).squeeze()
                # online_priora /= online_priora.sum(dim=-1, keepdim=True)

                online_prior_next = self.online_prior.nets[0](
                    next_states).squeeze()
                online_prior_next /= online_prior_next.sum(
                    dim=-1, keepdim=True)

            else:
                online_priora = torch.ones(self.batch_size, self.nA, device=self.device) * (1/self.nA)
                online_prior_next = torch.ones(self.batch_size, self.nA, device=self.device) * (1/self.nA)


            online_chi = (
                online_u_next * online_prior_next.repeat(self.num_nets, 1, 1)).sum(dim=-1)
            ...
            target_next_u = self.aggregator_fn(online_u_next, dim=0)

            next_chis = (target_next_u * online_prior_next).sum(dim=-1)
        ...
        if self.use_rawlik:
            ...
            pistar = self.aggregator_fn(online_curr_ua, dim=0) * online_priora
...
def main():
    ...
    agent = NoTargetUAgent(env_id, **config, device='cuda', log_interval=1000,
