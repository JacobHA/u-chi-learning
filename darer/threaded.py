...
class LogULearner:
    def __init__(self,
        ...
        self.train_thread = threading.Thread(target=self.train_threaded, args=(total_timesteps,))
        self.data_collection_thread = threading.Thread(target=self.collect_data_threaded)
    ...
    def train_threaded(self, total_timesteps, beta_schedule=None):
    ...
    def collect_data_threaded(self, n_episodes=1):
...
def main():
    ...
    agent = LogULearner(env_id, **config, device='cpu', log_interval=1500,
                        log_dir='pend', num_nets=2, render=0, aggregator='max',
                        scheduler_str='exponential', algo_name='dt-max', beta_end=10)
