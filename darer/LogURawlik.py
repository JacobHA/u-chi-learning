...
class LogULearner:
    ...
    def train(self):
        ...
        for grad_step in range(self.gradient_steps):
            ...
            with torch.no_grad():
                ...
                prior = torch.exp(self.prior.forward(next_states))
                # normalize:
                prior /= prior.sum(dim=-1, keepdim=True)
                log_prior = torch.log(prior)

                log_ref_chi = torch.logsumexp(log_prior + ref_logu_next,dim=-1)
                ...
                next_logus = torch.logsumexp(log_prior + target_next_logus, dim=-1)
        ...
        while self.env_steps < total_timesteps:
            ...
            while not done and self.env_steps < total_timesteps:
                ...
                if self.env_steps < self.learning_starts:
                    ...
                    with torch.no_grad():
                        prior = torch.exp(self.prior.forward(state))
                        prior /= prior.sum()
                    action = self.online_logus.choose_action(state, prior=prior)
    ...
    def _log_stats(self):
        if self.env_steps % self.log_interval == 0:
            ...
            if self.env_steps >= 0:
                self.avg_eval_rwd = self.evaluate(10)
        ...
        for ep in range(n_episodes):
            ...
            while not done:
                with torch.no_grad():
                    prior = torch.exp(self.prior.forward(state))
                    prior /= prior.sum()
                action = self.online_logus.greedy_action(state, prior=prior)
...
def main():
    ...
    agent = LogULearner(env_id, **config, device='cuda', log_interval=5000,
                        ...
                        scheduler_str='none', algo_name='std', beta_end=20)
    # Measure the time it takes to learn:
    t0 = time.thread_time_ns()
    agent.learn(total_timesteps=5_000_000, beta_schedule='linear')
